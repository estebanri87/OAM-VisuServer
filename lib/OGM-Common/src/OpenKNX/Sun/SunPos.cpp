...
void sunpos(cTime udtTime, cLocation udtLocation, cSunCoordinates *udtSunCoordinates)
{
    ...
    // (i.e., the result may be greater than 2*Pi)
    {
        ...
        dEclipticLongitude = dMeanLongitude + 0.03338320972 * sin(dMeanAnomaly) + 0.0003497596876 * sin(2 * dMeanAnomaly) - 0.0001544353226 - 0.00000868972936 * sin(dOmega);
        dEclipticObliquity = 0.4090904909 - 6.213605399e-9 * dElapsedJulianDays + 0.00004418094944 * cos(dOmega);
    }
    ...
    // Calculate local coordinates ( azimuth and zenith angle ) in degrees
    {
        ...
        dGreenwichMeanSiderealTime = 6.697096103 +
                                     0.06570984737 * dElapsedJulianDays + dDecimalHours;
