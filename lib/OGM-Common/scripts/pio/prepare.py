...
def _resolve_git_command():
  ...
  if git_in_path:
    _git_cmd = git_in_path
    return _git_cmd

  # Common Windows fallback locations when git is not available in PATH.
...
def get_git_version(path):
  ...
  if not git_cmd:
    if not _git_warned:
      print("{}  Warning: git executable not found. Falling back to 'nogit'.{}".format(console_color.YELLOW, console_color.END))
      _git_warned = True
    return "nogit"

  try:
    result = subprocess.run([git_cmd, "rev-parse", "--short", "HEAD"], capture_output=True, text=True, cwd=path, check=True)
